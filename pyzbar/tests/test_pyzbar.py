...
class TestDecode(unittest.TestCase):
    ...
    def test_unsupported_bits_per_pixel(self):
        # 16 bits-per-pixel
        # Rho-9 note: upstream derived a bpp value and raised a generic
        # PyZbarError with an "Unsupported bits-per-pixel" message. This
        # fork rejects the same input earlier, via the exact
        # buffer-length-vs-width*height check (PyZbarValidationError is a
        # PyZbarError subclass, so both are still "rejected as a
        # PyZbarError" from a caller's point of view - only the specific
        # message/subclass changed).
        data = (list(range(3 * 3 * 2)), 3, 3)
        self.assertRaisesRegex(
            PyZbarError,
            (
                r'Pixel buffer length 18 does not exactly match width x '
                r'height \(9\) at the only supported bit depth \(8bpp\)'
            ),
            decode, data
        )
    ...
    def test_inconsistent_dimensions(self):
        ...
        # Rho-9 note: message changed (see note above) - still rejected
        # as a PyZbarError (specifically PyZbarValidationError).
        data = (list(range(10)), 3, 3)
        self.assertRaisesRegex(
            PyZbarError,
            (
                r'Pixel buffer length 10 does not exactly match width x '
                r'height \(9\) at the only supported bit depth \(8bpp\)'
            ),
            decode, data
        )
