...
Rho-9 note: this module is deliberately left as a faithful, unmodified
binding layer (matching upstream pyzbar 0.1.9). It only declares function
signatures - it never touches attacker-controlled data itself. All input
validation and sanity caps live in pyzbar.py, at the points where
attacker-controlled bytes/dimensions/lengths actually get constructed into
ctypes calls or read back out of zbar's C structures.
"""
from ctypes import (
...
try:
    zbar_symbol_get_orientation = zbar_function(
        ...
        # Rho-9 fix: this was declared c_uint upstream, but zbar's C
        # signature returns a *signed* int (zbar_orientation_t has
        # ZBAR_ORIENT_UNKNOWN = -1). With c_uint, that -1 comes back as
        # 4294967295 instead, which does not correspond to any value in
        # ZBarOrientation and raises an uncaught ValueError deep in
        # decode() - reachable by any attacker-supplied image where
        # zbar's orientation heuristic reports "unknown" (confirmed via
        # adversarial testing with structured-append QR symbols, which
        # reliably trigger this). c_int fixes the sign so -1 round-trips
        # correctly as -1, matching ZBarOrientation.UNKNOWN.
        c_int,
        POINTER(zbar_symbol)
    )
